...
public class PlayingField {
    ...
    //Constructor
    /**
     * Default constructor that creates a new {@link PlayingField} and the {@code controlArray}
     */
    public PlayingField() {
        controlArray = new boolean[height][width];
        board = new Card[height][width];
        //All cards are set to starting Position
        /* A boolean array is always initiated with 'false'
     ...
     * Checks whether the row and the col are within the effective range.
     * 0 < row <= height - 1.
     * 0 < col <= width - 1.
     * If not, throw out a IllegalArgumentException.
     * Checks whether the card has been already opened.
     * If that is the case, a IllegalArgumentException is thrown.
     * @param row Row position of the selected card
     * @param col Column position of the selected card
    ...
    public Card getCard(int row, int col){
        if(row > height - 1 || col > width - 1 || height < 0 || width < 0){
            throw new IllegalArgumentException("The row or col can not be bigger than height/width or less than 0!");
        }
        if(controlArray[row][col] == true){
            throw new IllegalArgumentException("This card has already been opened!");
        }
     ...
     * Therefore there is no need to check whether row and col are within the effective range.
